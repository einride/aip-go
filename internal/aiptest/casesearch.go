...
func (r *resourceGenerator) searchCase() testCase {
	_, ok := r.resource.Methods[aipreflect.MethodTypeSearch]
	if !ok {
	...
	return newTestCase("Search", func(f *codegen.File) {
		...
		if hasParent(r.resource) {
			f.P("// search methods with a specified parent should not return resources")
			f.P("// owned by another parent.")
			f.P("t.Run(\"isolation\", func(t *", testing, ".T) {")
			f.P("a.maybeSkip(t)")
			r.callSearch(f, callSearchOpts{
				parent:        "parent02",
				pageSize:      "9999",
				response:      "response",
				assertNoError: true,
